...
public class Gujikja {
    ...
    String passwd;      // 암호
    String name;        // 성명
    String jubun;       // 주민등록번호 앞의 7자리
                        // 예 : "9501151" 남자     "0105103" 남자
                        // 예 : "9501152" 여자     "0105104" 여자
    static int count;

    // method 생성 (==> field(속성)을 어떤 방식으로 처리하고자 할때 메소드를 생성)
    ...
    int getAge() {
        ...
        if (jubun.substring(6).equals("1") || jubun.substring(6).equals("2")) {
            birthYear = Integer.parseInt("19"+jubun.substring(0,2));
        } else if (jubun.substring(6).equals("3") || jubun.substring(6).equals("4")) {
            birthYear = Integer.parseInt("20"+jubun.substring(0,2));
        } else {
