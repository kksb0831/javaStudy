...
//                }
                ...
                for (;;) {
                    ...
                    if (oddNum%2 == 0) {  // 첫번째 입력받은 값이 짝수라면
                        oddNum = oddNum + 1;// num1 => 2 (짝수)
                    }
                    if (oddNum <= num2) {
                        oddSum += oddNum;
                ...
                // == 짝수의 합을 구한다 ==
                for (;;) {
                    // 짝수의 합을 구한는데 첫번째로 입력한 값이
                    // num1 -> 1 (홀수) 이라면
                    // num1은 1을 더한 후(즉, 2으로 만듬) 2씩 증가시켜 합해주면 된다.

                    // 짝수의 합을 구한는데 첫번째로 입력한 값이
                    // num1 -> 2 (짝수) 이라면
                    // num1을 2씩 증가시켜 합해주면 된다.

                    if (evenNum%2 != 0) {  // 첫번째 입력받은 값이 짝수라면
                        evenNum = evenNum + 1;// num1 => 2 (짝수)
                    }
                    if (evenNum <= num2) {
                        evenSum += evenNum;
